...
def main(args):
    ...
    if args.representation_type == "initial":
        rep_dir = os.path.join(output_dir, args.representation_type)
    ...
    group=f"k_{args.K}_a_{args.ALPHA}_m_0_{args.FRAC_INPUT}_frac_n_{args.select_frac_neurons}_b1e7_poisson",
    ...
    if ds_pooling != "none":
        ...
        base_dir  = os.path.join(sweep_dir, args.representation_type, f"sel_frac_{args.select_frac_neurons}", f"{args.learned_pooling}_p", f"sweep-{sweep_id or 'nosweep'}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--FRAC_INPUT", type=str, default="00")
